...
class qsub_hpc:
    ...
    def write_sh(self,cmd,jobname,sidx,ncpu,h,m,mem,email,wdir,mo,pre,post,a):
        ...
        oup.write("#PBS -l nodes=1:ppn=")
        oup.write("%i,walltime=%i:%i:00,mem=%igb\n" % (ncpu,h,m,mem))
        if email != "":
            oup.write("#PBS -M %s\n" % email)
            oup.write("#PBS -m a\n")
        if a != "":
    ...
    def submit(self,jobs,sidx,wtime,mem,jobname,ncpu,email,logdir,a,wdir="",
              module="",pre="",post=""):
        runtype = 0
        if type(jobs) != list:
            runtype = 1
        ...
        for i in jobs:
            if i.strip() != '' and i[0] != "#":
                print "  job %i" % sidx
                self.write_sh(i,jobname,sidx,ncpu,h,m,mem,email,wdir,mo,pre,post,a)
                os.system("chmod 755 %s%i.sh" % (jobname,sidx))
                os.system("qsub %s%s%i.sh"    % (logdir,jobname,sidx))  
                sidx += 1
    
    def queue(self,jcommand,stime,nsub,juser,jrange,wtime,mem,jobname,ncpu,email,
            logdir,a,wdir="",module="",pre="",post=""):
        jdict = {}
        inp = open(jcommand)
        jobs = inp.readlines()
        if jrange != "*":
        ...
        while j < len(jobs): 
            ...
            #####
            rint = randint(1,1e6)
            os.system("qstat -u %s > TMP.%i" % (juser,rint))
            time.sleep(2)
            inp = open("TMP.%i" % rint)
            inl = inp.readlines()
            if len(inl) != 0:
            ...
            if currj < nsub:
                ...
                self.submit(jseg,j+1,wtime,mem,jobname,ncpu,email,logdir,a,wdir,module)
                j += len(jseg)
            else:
                oup.write("waiting\t")
                print "waiting\t"
...
if __name__ == '__main__':
    ...
    if f == "submit":
        if "" in [c]:
            ...
            qsub.help()
        qsub.submit(c,0,w,m,J,p,e,o,a,wd,mo,pre,post)
    elif f == "queue":
        if "" in [c,u]:
            print "\nNeed cmd line file, user name\n"
            qsub.help()
        qsub.queue(c,s,n,u,r,w,m,J,p,e,o,a,wd,mo,pre,post)
    elif f == "qdel":
        if r == "" and u == "":
	...
	def submit(self,jobs,sidx,wtime,mem,jobname,ncpu,email,logdir,wdir="",
              module="",pre="",post=""):
		runtype = 0
		if type(jobs) != list:
			runtype = 1
		...
		for i in jobs:
			if i.strip() != '' and i[0] != "#":
				print ("  job %i" % sidx)
				self.write_sh(i,jobname,sidx,ncpu,h,m,mem,email,wdir,mo,pre,post)
				os.system("chmod 755 %s%i.sh" % (jobname,sidx))
				os.system("qsub %s%s%i.sh"    % (logdir,jobname,sidx))	
				sidx += 1
	
	def queue(self,jcommand,stime,nsub,juser,jrange,wtime,mem,jobname,ncpu,email,
            logdir,wdir="",module="",pre="",post=""):
		jdict = {}
		inp = open(jcommand)
		jobs = inp.readlines()
		if jrange != "*":
		...
		while j < len(jobs): 
			...
			if currj < nsub:
				...
				oup.write("submit %i\t" % len(jseg))
				print ("current %i, submit %i\n" % (currj,len(jseg)))
				self.submit(jseg,j+1,wtime,mem,jobname,ncpu,email,logdir,wdir,module)
				j += len(jseg)
			else:
				oup.write("waiting\t")
				print ("waiting\t")
...
if __name__ == '__main__':
	...
	if f == "submit":
		if "" in [c]:
			print ("\nNeed cmd line file, user name\n")
			qsub.help()
		qsub.submit(c,0,w,m,J,p,e,o,wd,mo,pre,post)
	elif f == "queue":
		if "" in [c,u]:
			print ("\nNeed cmd line file, user name\n")
			qsub.help()
		qsub.queue(c,s,n,u,r,w,m,J,p,e,o,wd,mo,pre,post)
	elif f == "qdel":
		if r == "" and u == "":
			print ("\nNeed range or user name\n")
			qsub.help()
